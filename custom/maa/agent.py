...
用法（通常由 maa-support-extension 自动启动，无需手动跑）：
    python -m custom.maa.agent <identifier>
...
from custom.maa.registry import register_all
from custom.utils.logger import setup_logging
...
def main() -> int:
    if len(sys.argv) < 2:
        print("用法: python -m custom.maa.agent <identifier>", file=sys.stderr)
        return 2
